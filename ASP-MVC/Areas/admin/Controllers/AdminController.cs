...
namespace ASP_MVC.Areas.admin.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Index(string searchStringPV, string searchStringCT, int page = 1, int pagesize = 20)
        {
            ...
            ViewBag.YCCT = listCT.OrderByDescending(x => x.Status).ToPagedList(page, pagesize);
        ...
        public ActionResult YCHoanThanh(string searchStringPV, string searchStringCT, int page = 1, int pagesize = 10)
        {
            ...
            if (!string.IsNullOrEmpty(searchStringCT))
            {
                ...
                ViewBag.YCCT = db.YeuCauPhucVus.Where(x => x.Status == 3 && x.Loai == 2).OrderByDescending(x => x.NgayHoanThanh).ToPagedList(page, pagesize);

            return View(model.ToPagedList(page,pagesize));
        }
        public ActionResult YCHuy(string searchStringPV, string searchStringCT, int page = 1, int pagesize = 10)
        {
            IQueryable<YeuCauPhucVu> model = db.YeuCauPhucVus;
            if (!string.IsNullOrEmpty( searchStringPV))
            {
            ...
            if (!string.IsNullOrEmpty(searchStringCT))
            {
                ...
                ViewBag.YCCT = db.YeuCauPhucVus.Where(x => x.Status == 4 && x.Loai == 2).OrderByDescending(x => x.NgayHoanThanh).ToPagedList(page, pagesize);
        ...
        public ActionResult ChangePass(string oldPass, string NewPass1, string NewPass2)
        {
            ...
            {
                string name = HttpContext.User.Identity.Name;
                var rs = db.NhanViens.SingleOrDefault(x => x.Username.Contains(name));
