...
init: function () {
    ...
    registerEvents: function () {
        ...
        $('.Opendialog').off('click').on('click', function () {
            ...
            bbnt.LuuCTBBNT(idbbnt, idkh, listCTBBNT);
    ...
    LuuCTBBNT: function (idbbnt, idkh, list) {
        $.ajax({
            url: "/admin/BienBanNghiemThu/LuuCTBBNT",
            async: false,
            data: { list:list, idkh: idkh, idbbnt: idbbnt },
            dataType: "json",
            type: "POST",
            success: function (response) {
                if (!response.status) {
...
resetForm: function () {
    ...
    ThemChiPhi: function (idyc) {
        ...
        if (tenCP != "" && soluong != "" && dongia != "" && dongia > 0 && soluong >0) {
            $.ajax({
                url: "/admin/BienBanNghiemThu/ThemChiPhi",
                data: { idyc: idyc, tenCP: tenCP, soluong: soluong, dongia: dongia, ghichu:ghichu },
                dataType: "json",
                type: "POST",
                success: function (response) {
                    if (response.status) {
